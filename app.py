...
def view_db():
    ...
    data = models.MainDB.query.order_by(models.MainDB.id).all()
...
def update_db():
    ...
    to_update = resp_dict.copy()
    del to_update['id']
    db_update = models.MainDB
    db.session.query(models.MainDB).filter(models.MainDB.id == int(resp_dict['id'])).update(to_update)
    db.session.commit()
